...
function tablaMultiplicar(numero) {
  // Usé un bucle for del 1 al 10 para multiplicar el número por cada valor
  // y agregué cada resultado al array con push()
  const tabla = [];
  for (let i = 1; i <= 10; i++) {
    tabla.push(numero * i);
  }
...
function numerosPares(n) {
  // Usé un bucle for empezando en 2 e incrementando de 2 en 2
  // para obtener solo números pares hasta n
  const pares = [];
  for (let i = 2; i <= n; i += 2) {
    pares.push(i);
  }
...
function filtrarMayores(numeros, limite) {
  // Usé un bucle for para recorrer el array y una condición if
  // para agregar solo los números mayores al límite al nuevo array
  const mayores = [];
  for (let i = 0; i < numeros.length; i++) {
    if (numeros[i] > limite) {
...
function crearMatriz(filas, columnas) {
  // Usé dos bucles for anidados: el externo crea las filas y el interno
  // llena cada fila con ceros según el número de columnas
  const matriz = [];
...
function sumaMatriz(matriz) {
  // Usé dos bucles for anidados para recorrer todas las filas y columnas
  // y fui acumulando cada elemento en la variable suma
  let suma = 0;
...
function obtenerColumna(matriz, indiceColumna) {
  // Usé un bucle for para recorrer todas las filas y extraer
...
function transponer(matriz) {
  // Usé dos bucles anidados: el externo recorre las columnas de la matriz original
  // y el interno recorre las filas para crear nuevas filas con elementos de cada columna
  const transpuesta = [];
